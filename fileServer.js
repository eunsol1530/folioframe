...
app.post("/read-number", (req, res) => {
  ...
  const absolutePath = path.resolve(__dirname, path.normalize(filePath)); // 절대 경로로 변환
...
app.post("/remove-from-file-end", (req, res) => {
  ...
  if (!filePath || typeof filePath !== 'string' || typeof numCharsToRemove !== 'number') {
...
app.post("/append-string", (req, res) => {
  ...
  if (!filePath || typeof filePath !== 'string' || typeof string !== 'string') {
    return res.status(400).json({ error: "Invalid input." });
  }
  const absolutePath = path.resolve(__dirname, path.normalize(filePath)); // 절대 경로로 변환
  if (!absolutePath.startsWith(__dirname)) {
...
app.post("/update-file", (req, res) => {
  ...
  if (!filePath || typeof filePath !== 'string' || typeof operation !== 'string' || typeof string !== 'string') {
    return res.status(400).json({ error: "Invalid input." });
  }
  const absolutePath = path.resolve(__dirname, path.normalize(filePath));
  if (!absolutePath.startsWith(__dirname)) {
    return res.status(400).json({ error: "Invalid file path." });
  }
...
app.post("/patch-hits", async (req, res) => {
  try {
    ...
    if (!filePath || typeof filePath !== 'string' || typeof projectId !== 'string' || typeof newHits !== 'number') {
      return res.status(400).json({ error: "Invalid input." });
    }
    const absolutePath = path.resolve(__dirname, path.normalize(filePath));
    if (!absolutePath.startsWith(__dirname)) {
...
app.post("/patch-hack-hits", async (req, res) => {
  try {
    ...
    if (!filePath || typeof filePath !== 'string' || typeof hackId !== 'string' || typeof newHits !== 'number') {
      return res.status(400).json({ error: "Invalid input." });
    }
    const absolutePath = path.resolve(__dirname, path.normalize(filePath));
    if (!absolutePath.startsWith(__dirname)) {
...
app.post("/patch-contacts", async (req, res) => {
  try {
    ...
    if (!filePath1 || typeof filePath1 !== 'string' || !filePath2 || typeof filePath2 !== 'string' || typeof projectId !== 'string' || typeof newContact !== 'string') {
      return res.status(400).json({ error: "Invalid input." });
    }
    const absolutePath1 = path.resolve(__dirname, path.normalize(filePath1));
    const absolutePath2 = path.resolve(__dirname, path.normalize(filePath2));
    if (!absolutePath1.startsWith(__dirname) || !absolutePath2.startsWith(__dirname)) {
      return res.status(400).json({ error: "Invalid file path." });
    }
...
app.post("/patch-likes", async (req, res) => {
  try {
    ...
    if (!filePath || typeof filePath !== 'string' || typeof projectId !== 'string' || typeof userId !== 'string') {
      return res.status(400).json({ error: "Invalid input." });
    }
    const absolutePath = path.resolve(__dirname, path.normalize(filePath));
    if (!absolutePath.startsWith(__dirname)) {
...
app.post("/patch-comments", async (req, res) => {
  try {
    ...
    if (!filePath || typeof filePath !== 'string' || typeof projectId !== 'string' || typeof commentId !== 'string') {
      return res.status(400).json({ error: "Invalid input." });
    }
    const absolutePath = path.resolve(__dirname, path.normalize(filePath));
    if (!absolutePath.startsWith(__dirname)) {
...
app.post("/patch-participant", async (req, res) => {
  try {
    ...
    if (!filePath || typeof filePath !== 'string' || typeof hackId !== 'string' || typeof userId !== 'string') {
      return res.status(400).json({ error: "Invalid input." });
    }
    const absolutePath = path.resolve(__dirname, path.normalize(filePath));
    if (!absolutePath.startsWith(__dirname)) {
...
app.post("/delete-object", (req, res) => {
  ...
  if (!filePath || typeof filePath !== 'string' || typeof idField !== 'string' || typeof id !== 'string') {
    return res.status(400).json({ error: "Invalid input." });
  }
  const absolutePath = path.resolve(__dirname, path.normalize(filePath));
  if (!absolutePath.startsWith(__dirname)) {
...
app.post("/update-user-field", (req, res) => {
  ...
  if (!filePath || typeof filePath !== 'string' || typeof idField !== 'string' || typeof id !== 'string' || typeof field !== 'string') {
    return res.status(400).json({ error: "Invalid input." });
  }
  const absolutePath = path.resolve(__dirname, path.normalize(filePath));
  if (!absolutePath.startsWith(__dirname)) {
